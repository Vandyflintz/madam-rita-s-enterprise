...
void main() {
  runApp(MyYearlySalesRecords());
}

class MyYearlySalesRecords extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      home: YearlySalesRecords(title: 'Madam Rita\'s Enterprise'),
    );
  }
}
...
class YearlySalesRecords extends StatefulWidget {
  YearlySalesRecords({Key? key, this.title, this.shopname, this.location})
      : super(key: key);
...
class _MyChartState extends State<YearlySalesRecords>
  ...
  _showfirstdate() {
    Future.delayed(Duration(seconds: 5)).then((value) {
      //showsnackbar(widget.shopname + " " + widget.location + " " + searchdate, "Okay");
      print(widget.shopname.toString() +
          " " +
            ...
            onSubmitted: (value) {
              setState(() {
                _future = DBProvider().fetchsearchedsoldProducts(searchdate, "",
                    "yearly", value, widget.shopname, widget.location);
    ...
    Future.delayed(Duration(seconds: 0)).then((value) {
      setState(() {
        _future = DBProvider().fetchsoldProducts(
            searchdate, "", "yearly", widget.shopname, widget.location);
        _individualfuture = DBProvider().fetchindividualsoldProducts(
            searchdate, "", "yearly", widget.shopname, widget.location, _user);
      });
                                              ...
                                              newsListSliver =
                                                  SliverGrid(
                                                    ...
                                                    delegate: SliverChildBuilderDelegate(
                                                        ...
                                                        double individualPrice = itemPricesList.fold(
                                                          0,
                                                              (previousValue, element) =>
                                                          previousValue! + (double.tryParse(element ?? '0') ?? 0),
                                                        );

                                                        // Update general worker price after a delay
                                                        Future.delayed(Duration(seconds: 1)).then((value) {
                                                          setState(() {
                                                            _generalworkerprice = totalprice! - individualPrice!;
                                                          });
                                                        });
                                                ...
                                                newsListSliver =
                                                    SliverGrid(
                                                      ...
                                                      delegate: SliverChildBuilderDelegate(
                                                          ...
                                                          // Update the individual worker price
                                                          Future.delayed(Duration(seconds: 0)).then((value) {
                                                            setState(() {
  ...
  _movetopreviousweek(String? odate) async {
    ...
    Future.delayed(Duration(seconds: 0)).then((evalue) {
      ...
      _individualfuture = DBProvider().fetchindividualsoldProducts(searchdate,
          enddate, "yearly", widget.shopname, widget.location, _user);
